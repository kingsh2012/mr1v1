...
本仓库是 **CS 1.6 1v1 对战平台服务端**，当前开发与部署都在 `mr1v1-rehlds/` 目录下（已部署完成的 ReHLDS 环境）。
...
位置：`mr1v1-rehlds/cstrike/addons/amxmodx/scripting/mr1v1_telemetry.sma`（编译产物 `mr1v1_telemetry.amxx`，已在 `configs/plugins.ini` 中注册）。
...
插件源码在 `mr1v1-rehlds/cstrike/addons/amxmodx/scripting/`，扩展名 `.sma`。
...
**配置维护：**
`server.cfg`、`amxx.cfg`、`metamod-plugins.ini` 等配置文件直接在 `mr1v1-rehlds/` 目录下手动维护，不经过任何配置渲染/注入工具。临时调试可用 `mr1v1-rehlds/set_cvar.sh <cvar> <value>` 修改 `server.cfg` 中已存在的cvar值（保留行尾注释）。
...
需使用 `PROCS.PRO-REHLDS-LINUX-PROD/libsteam_api.so`（新版 Steam SDK），已复制到 mr1v1-rehlds/ 目录。
...
### 插件实现技术路线
插件优先使用 **ReAPI**（5.29.0.358，include 已安装在 `mr1v1-rehlds/cstrike/addons/amxmodx/scripting/include/`）实现游戏事件 hook、读写实体/玩家数据、调用游戏内部函数等。只有 ReAPI 完全无法覆盖的功能才退回 fakemeta/engine 等旧方式。
